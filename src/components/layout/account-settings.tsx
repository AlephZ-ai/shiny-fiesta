...
import { UPDATE_SERVICE_ADVISOR_MUTATION } from "@/graphql/mutations";

import { Text } from "../text";
import CustomAvatar from "../custom-avatar";

import {
  UpdateServiceAdvisorMutation,
  UpdateServiceAdvisorMutationVariables,
} from "@/graphql/types";
...
export const AccountSettings = ({ opened, setOpened, userId }: Props) => {
  ...
  const { saveButtonProps, formProps, queryResult } = useForm<
     ...
     */
    GetFields<UpdateServiceAdvisorMutation>,
    // a type that represents an HTTP error. Used to specify the type of error mutation can throw.
    HttpError,
    // A third type parameter used to specify the type of variables for the UpdateServiceAdvisorMutation. Meaning that the variables for the UpdateServiceAdvisorMutation should be of type UpdateServiceAdvisorMutationVariables
    GetVariables<UpdateServiceAdvisorMutationVariables>
  >({
    /**
    ...
    meta: {
      ...
      gqlMutation: UPDATE_SERVICE_ADVISOR_MUTATION,
    },
  });
